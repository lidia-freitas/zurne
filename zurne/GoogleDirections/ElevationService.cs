...
namespace GoogleDirections
{
  ...
  /// </summary>
  public class ElevationService : HttpWebService
  {
    /// <summary>
    ...
    {
      var req = string.Format("{2}&locations={0},{1}", location.Latitude, location.Longitude, BaseUrl());
    ...
    public IEnumerable<double> GetElevation(IEnumerable<LatLng> locations)
    {
      ...
      foreach (var ll in locations)
      {
        if (req != "")
          req += "|";
        req += string.Format("{0},{1}", ll.Latitude.ToString("F6"), ll.Longitude.ToString("F6"));
      }
      ...
      {
        elevations.Add(double.Parse(node.InnerText));
      }
